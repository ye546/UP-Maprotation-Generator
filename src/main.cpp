...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		case WM_CREATE:
			...
			field2 = CreateWindowEx(WS_EX_CLIENTEDGE, "Edit", "excluded maps.txt", WS_CHILD | WS_VISIBLE, 10, 40, 150, 25, hWnd, NULL, hInst, (LPVOID)lParam);	//old rotation
			...
			static2 = CreateWindowEx(NULL, "Static", "Excluded", WS_CHILD | WS_VISIBLE | WS_BORDER | SS_CENTER, 550, 10, 90, 20, hWnd, NULL, hInst, (LPVOID)lParam);	//old rotation
			...
			textarea2 = CreateWindowEx(NULL, "Edit", "", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | ES_MULTILINE, 500, 40, 200, 400, hWnd, NULL, hInst, (LPVOID)lParam);	//old rotation

			//to allow scrolling if you add a lot of maps
			//that you want to exclude
			SendMessage(textarea2,EM_SETREADONLY, 1, 0);
...
void save(vector &v) {
	std::ofstream file("New Rotation.txt", std::fstream::app);
	...
	file2.open("holder3.txt");

	//fill the vector like usual
	std::string tmp;
	while (std::getline(file2, tmp))
		v.push_back(tmp);
	...
	//ISSUE NOTCED: THE READING FROM THE FILE IS READ AS 1 STRING, THIS IS NOT INTENDED.
...
void RandomizeNewRotation(vector &all_maps, vector &old_maps) {
	...
	std::ofstream file3("holder3.txt");

	//the writing to the file

	//for displaying purposes
	for (int i = 0; i < new_rotation.size(); i++) { 
		file1 << new_rotation[i] << '\r';
	}
	//for displaying purposes
	for (int i = 0; i < old_maps.size(); i++) {
		file2 << old_maps[i] << '\r';
	}
	//actual rotation format
	for (int i = 0; i < new_rotation.size(); i++) {
		if (&new_rotation[i] == &new_rotation.back()) {
			file3 << new_rotation[i];
		}
		else
			file3 << new_rotation[i] << " map ";
	}		
}
